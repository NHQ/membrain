...
args.add_option('-d', '--dtype', dest='dtype', default='float32', type='string')
opts, args = args.parse_args()
file = os.path.basename(opts.filename)

y, sr = librosa.load(opts.filename, sr=opts.rate, mono=True, dtype=opts.dtype)
sampleRate = opts.rate

data = {}
data['files'] = []

track = opts.dir + '/track.raw'
open(track, 'w').write(struct.pack('f'*y.size, *y)) 
